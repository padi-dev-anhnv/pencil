    ...
    <ul class="listtable">
      ...
      <li>
        <ul v-for="user in users">
          ...
          <li>{{user.username}}</li>
          <li>{{user.office.name}}</li>
          <li>
            <a :href="'/user/' + user.id + '/edit'" class="editbtn">
              <span>編集</span>
            </a>
            <label for="popup_delete" class="deletebtn" @click="confirmDelete(user)">
              <span>削除</span>
            </label>
          </li>
        </ul>
          ...
          <div class="popup_ctt">
            <div class="popup_cttinner">
              ...
              <ul class="btn_box btn2box">
                <li>
                  <label for="popup_delete" class="mainbtn" @click="doDelete">はい</label>
                </li>
                <li>
                  <label for="popup_delete" class="mainbtn subbtn">いいえ</label>
                </li>
              </ul>
            </div>
          </div>
...
export default {
  ...
  methods: {
    ...
    doDelete() {
      if (this.deleteId) {
        axios.post("/user/delete", { id: this.deleteId }).then(result => {
            this.loadUser();
        });
      }
    }
